...
class modUsersLatestHelper
{
	...
	static function getUsers($params)
	{
		...
		$query->select('a.id, a.name, a.username, a.registerDate');//a. per evitare conflitti se viene effettuata una Join con mysql

		//$query->select('a.id')
		//$query->select('a.name') forma riconsciuta
		//$query->select('a.username')
		//$query->select('a.registerDate')
