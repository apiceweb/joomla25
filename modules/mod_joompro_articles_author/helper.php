...
abstract class modJoomProArticlesAuthorHelper
{
	public static function getList(&$params)
	{
		...
		$option	= JRequest::getCmd('option'); //recupero commando pasato via url
		$view = JRequest::getCmd('view');
		if ($option == 'com_content' && $view == 'article') //identifica il componente (in questo caso il componente content nella visualizzazione article
		{
			// Get the dbo
			$db = JFactory::getDbo();
			...
			$now = $date->toMySql(); //altro database inserire forma corretta
			...
			$query->select('id, created_by, created_by_alias'); //cosa ricerchiamo
			...
			$query->clear();
			$query->select('a.*');
			$query->select('c.access AS cat_access, c.published as cat_state, c.alias as cat_alias'); //c.=a.
			...
			$query->where('(a.publish_down = '.$db->Quote($nullDate).' OR a.publish_down >= '.$db->Quote($now).')');//quote proteggela query
